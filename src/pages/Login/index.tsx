...
const LoginPage = () => {
  ...
  const handleSendCode = () => {
    ...
    formRef.current
      ...
      .then(async (values: any) => {
        ...
        const { code, message: errorMsg } = await getVerifyCode(values.account);
        if (code !== 0) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      // 登录
      const { data, code } = await accountLogin(account, verifyCode);
    ...
    <div className={styles['login-wrapper']}>
      <div className={styles['login-content']}>
        ...
        >
          ...
          >
            {<Input placeholder="请输入手机号" prefix={<MobileTwoTone />} allowClear />}
          </FormItem>
          <FormItem
            style={{ marginTop: 5 }}
            name="verifyCode"
          ...
          >
            {
              <Input
                ...
                autoComplete="off"
                addonAfter={
                  <Button type="primary" disabled={codeCountDown > 0} onClick={handleSendCode}>
                    获取验证码
          ...
          <Form.Item>
            <Button loading={loadingBtn} type="primary" htmlType="submit" className="login-button">
              登录
            </Button>
          </Form.Item>
        </Form>
